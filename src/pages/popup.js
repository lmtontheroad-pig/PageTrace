(function () {
  ...
  function renderStats(stats) {
    const semanticTotal = Number(stats.semanticTotal || 0);
    statusElement.textContent = stats.enabled
      ? `当前页面已启用，语义分类 ${semanticTotal}`
      : "当前页面已暂停";
    totalElement.textContent = String(stats.total);
    countsElement.replaceChildren();

    const enabledKeywords = stats.keywords.filter((keyword) => keyword.enabled);
    const enabledSemanticRules = (stats.semanticRules || []).filter((rule) => rule.enabled);
    if (enabledKeywords.length === 0 && enabledSemanticRules.length === 0) {
      const empty = document.createElement("p");
      empty.className = "kw-empty-text";
      empty.textContent = "没有启用的关键词或语义规则。";
    ...
    if (enabledKeywords.length > 0) {
      ...
      heading.className = "kw-count-heading";
      heading.textContent = "关键词";
      countsElement.appendChild(heading);
    }
    ...
    if (enabledSemanticRules.length > 0) {
      const heading = document.createElement("div");
      heading.className = "kw-count-heading";
      heading.textContent = "语义分类";
      countsElement.appendChild(heading);
    }

    enabledSemanticRules.forEach((rule) => {
      const row = document.createElement("div");
      row.className = "kw-count-row";
